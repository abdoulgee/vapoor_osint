/**
 * Sidebar — SOC-style slim navigation with neon green indicators.
...
export default function Sidebar() {
    ...
    return (
        <aside
            ...
            className={`fixed left-0 top-0 h-full z-40 flex flex-col bg-soc-900 border-r border-soc-700/60 transition-all duration-200 ${expanded ? 'w-44' : 'w-14'
                }`}
        >
            {/* Logo */}
            <div className="flex items-center gap-2 px-3 h-10 border-b border-soc-700/60 overflow-hidden">
                <div className="w-8 h-8 min-w-[2rem] flex items-center justify-center text-cyber-400 text-sm font-bold font-mono">
                    ◈
                </div>
            ...
            {/* Footer */}
            <div className="px-3 py-2 border-t border-soc-700/60 overflow-hidden">
                <p className={`text-[8px] font-mono text-soc-600 tracking-widest transition-opacity duration-150 ${expanded ? 'opacity-100' : 'opacity-0'}`}>
                    v2.0 // SOC
                </p>
            </div>
        </aside>
